...
from Quest.books_genres import genres, menu_categories
...
def worker_only(f):
    @wraps(f)
    def decorated_fun(*args, **kwargs):
        if current_user.privileges == "Worker":
            return f(*args, **kwargs)
        else:
            return abort(403)
...
def admin_and_worker_only(f):
    @wraps(f)
    def decorated_fun(*args, **kwargs):
        if current_user.privileges == "Worker" or current_user.privileges == "Admin":
            return f(*args, **kwargs)
        else:
            return abort(403)
...
def session_settings():
    ...
    if db.session.query(User).filter_by(login="client").first() is None:
        ...
        db.session.add(test)
    if db.session.query(User).filter_by(login="worker").first() is None:
        worker_user = User(login="worker", password=generate_password_hash("worker"),
                    first_name="worker", last_name="worker",
                    email="worker@worker.worker", privileges="Worker")
        worker = Worker()
...
def make_order():
    ...
    if current_user.is_authenticated and current_user.privileges == "User":
        ...
        order = Order(date_of_order=order_time, total_price=0, total_number_of_items=0,
                      client=db.session.query(User).filter_by(login="default").first().client)
...
@admin_and_worker_only
def check_stock():
    all_in_stock = db.session.query(Stock).all()
    return render_template("account/stock.html", products=all_in_stock)


@app.route("/magazyn/szczegoly/<int:stock_id>")
@login_required
@admin_and_worker_only
def check_stock_details(stock_id):
    article = Stock.query.get(stock_id)
    return render_template("account/stock_details.html", article=article, menu_categories=menu_categories, genres=genres)


@app.route("/edytuj_pozycje_menu/<int:menu_id>/<string:referrer>", methods=["GET", "POST"])
@login_required
@admin_and_worker_only
def edit_menu_item(menu_id, referrer):
    item_to_edit = Menu.query.get(menu_id)
    ...
    if edit_form.validate_on_submit():
        ...
        item_to_edit.stock.number_in_stock = edit_form.number_in_stock.data
        db.session.commit()
        if referrer == 'check_stock_details':
            return redirect(url_for('check_stock_details', stock_id=item_to_edit.stock.id))
        else:
...
@app.route("/ksiegarnia/edytuj_ksiazke/<int:book_id>/<string:referrer>", methods=["GET", "POST"])
@login_required
@admin_and_worker_only
def edit_book_for_sell(book_id, referrer):
    ...
    if edit_form.validate_on_submit():
        entered_number_in_stock = edit_form.number_in_stock.data

        entered_price = edit_form.price.data
        entered_discount = edit_form.discount.data
        ...
        entered_title = edit_form.title.data
        entered_author = edit_form.author.data
        entered_publisher = edit_form.publisher.data
        entered_genre = edit_form.genre.data
        entered_description = edit_form.description.data
        entered_publish_date = edit_form.publish_date.data
        entered_image_url = edit_form.image_url.data
        ...
        db.session.commit()
        if referrer == "check_stock_details":
            return redirect(url_for('check_stock_details', stock_id=edited_book.stock.id))
        else:
            return redirect(url_for(referrer))
    return render_template("bookshop/book_item.html", form=edit_form)
